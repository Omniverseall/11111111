            ...
            <ul className="space-y-4">
              {turnkeyFeatures.map((featureKey) => (
                <li key={featureKey} className="flex items-start space-x-3">
                  <CheckCircle className="w-6 h-6 text-tech-cyan mt-1 flex-shrink-0" />
          ...
          <div className={`tech-card ${isInView ? 'animate-fade-in' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
            ...
            <div className="space-y-4 mb-8">
              <div className="flex items-center space-x-4">
                <Mail className="w-5 h-5 text-tech-cyan flex-shrink-0" />
                <a href={`mailto:${t('contactEmail')}`} className="text-themed-foreground hover:text-tech-cyan transition-colors break-all">{t('contactEmail')}</a>
              </div>
              <div className="flex items-center space-x-4">
                <Phone className="w-5 h-5 text-tech-cyan flex-shrink-0" />
                <a href={`tel:${t('contactPhone').replace(/[\s-()]/g, '')}`} className="text-themed-foreground hover:text-tech-cyan transition-colors">{t('contactPhone')}</a>
              </div>
              <div className="flex items-center space-x-4">
                <UserCheck className="w-5 h-5 text-tech-cyan flex-shrink-0" />
