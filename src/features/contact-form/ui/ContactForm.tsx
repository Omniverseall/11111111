    ...
    <section id="contact-section" ref={sectionRef} className="section bg-themed-surface">
      <div className="container">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className={`card ${isInView ? 'animate-fade-in' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
            <h3 className="text-2xl font-semibold mb-6 text-center">{t('contactInfo')}</h3>
            <div className="space-y-4 mb-8">
              <div className="flex items-center space-x-4">
                <Mail className="w-5 h-5 text-green-400 flex-shrink-0" />
                <a href={`mailto:${t('contactEmail')}`} className="text-themed-foreground hover:text-green-400 transition-colors break-all">{t('contactEmail')}</a>
              </div>
              <div className="flex items-center space-x-4">
                <Phone className="w-5 h-5 text-green-400 flex-shrink-0" />
                <a href={`tel:${t('contactPhone').replace(/[\s-()]/g, '')}`} className="text-themed-foreground hover:text-green-400 transition-colors">{t('contactPhone')}</a>
              </div>
              <div className="flex items-center space-x-4">
