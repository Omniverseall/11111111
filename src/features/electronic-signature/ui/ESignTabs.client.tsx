...
export const ESignTabsClient: React.FC<{ dict: Dictionary }> = ({ dict }) => {
  ...
  const tabs = [
    { key: 'sign', label: dict.eSignTabSign, icon: <FileCheck className="w-8 h-8 text-tech-cyan" />, title: dict.eSignTabSignTitle, desc: dict.eSignTabSignDesc },
    { key: 'verify', label: dict.eSignTabVerify, icon: <Shield className="w-8 h-8 text-tech-teal" />, title: dict.eSignTabVerifyTitle, desc: dict.eSignTabVerifyDesc },
    ...
    <>
      <div className="tabs flex mb-2 space-x-1 bg-themed-surface border border-themed-border p-1 rounded-lg">
        {tabs.map((tab, index) => (
          <button
            key={tab.key}
            className={`flex-1 py-2.5 px-4 rounded-md transition-colors text-sm font-semibold ${activeTab === index ? 'bg-tech-cyan text-black' : 'text-themed-muted hover:text-themed-foreground'}`}
