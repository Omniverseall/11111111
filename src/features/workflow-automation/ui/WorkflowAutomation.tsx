...
export const WorkflowAutomation: React.FC<{ dict: Dictionary }> = ({ dict }) => {
  const sections = [
    {
      icon: <Vote className="w-6 h-6 text-tech-cyan" />,
    ...
    {
      icon: <Settings className="w-6 h-6 text-tech-teal" />,
    ...
    {
      icon: <FileSearch className="w-6 h-6 text-tech-light-blue" />,
    ...
    {
      icon: <Database className="w-6 h-6 text-tech-blue" />,
    ...
    <section id="processes-section" className="section bg-themed-surface">
      <div className="container">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h3 className="text-base font-semibold text-tech-cyan uppercase tracking-wider mb-2">{dict.workflowSectionSubtitle}</h3>
          ...
          {sections.map((section, index) => (
            <div key={index} className="tech-card">
              <div className="mb-6 flex items-center space-x-4">
                <div className="w-12 h-12 bg-tech-cyan/10 flex items-center justify-center rounded-lg flex-shrink-0">
                  {section.icon}
                </div>
